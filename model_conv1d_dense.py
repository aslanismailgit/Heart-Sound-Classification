...
model_1d_1.summary()
#%%
dense_input = keras.Input(shape=(50, 200), name="dense_m")
d = layers.Reshape((10000,1))(dense_input)
...
d = layers.BatchNormalization()(d)
d = layers.Dense(512, activation='relu')(d)
d = keras.layers.GlobalAveragePooling1D()(d)
dense_output = layers.BatchNormalization()(d)
...
# d = keras.layers.Dropout(0.05)(d)
# d = keras.layers.GlobalAveragePooling1D()(d)
# dense_output = keras.layers.Dense(10)(d)

model_dense = keras.Model(inputs=dense_input, outputs=dense_output)
model_dense.summary()
#%%
outModels = [conv1d_output_1,dense_output]
inModels = [conv1d_input_1,dense_input]

concatted = keras.layers.Concatenate()(outModels)

c = layers.Dense(256, activation="relu")(concatted)
...
model = keras.models.Model(inputs = inModels ,outputs = result)
model.summary()
#%%
...
shortDisc = "(10000x1 conv1d ksize 9 Flatten + paralel dense) => dense "
y_pred_prob_test = model.predict([x_test,x_test])
modelname = ""
...
#%%
y_pred_prob_val = model.predict([x_val,x_val])
DataSource = "Val"
...
#%%
y_pred_prob_train = model.predict([x_train, x_train])
DataSource = "Train"
